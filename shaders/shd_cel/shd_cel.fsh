...
void main()
{
	float diffuse_ratio1 = max(dot(v_vNormal, u_diffuse_direction1), 0.0) * u_normal_weight;
	float diffuse_ratio2 = max(dot(v_vNormal, u_diffuse_direction2), 0.0) * u_normal_weight;
